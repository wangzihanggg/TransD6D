...
def view_labels(rgb_chw, img_id, obj_id, cld_cn, labels, K=config.intrinsic_matrix['linemod']):
    ...
    rgb_hwc = cv2.imread('datasets/linemod/Linemod_preprocessed/data/' + str(obj_id) + '/rgb/' + img_id + '.png')
...
class Trainer(object):
    ...
    def eval_epoch(self, d_loader, epoch, is_test=False, test_pose=False):
        ...
        for _, data in tqdm.tqdm(enumerate(d_loader), total=len(d_loader)):
        ...
        for start_epoch in tqdm.tqdm(range(n_epochs)):
            ...
            for batch in tqdm.tqdm(train_loader):
                ...
                if it % self.plot_frequency == 0:
                    self.loss_history["train"].append(loss.cpu().detach().numpy())
                    self.plot_losses()
    ...
    def plot_losses(self):
        ...
        plt.plot(self.loss_history["train"], label="Train Loss", marker='o')
        plt.plot(self.loss_history["val"], label="Validation Loss", marker='o')
...
def train():
    # 打印当前进程的本地rank，通常在多GPU训练中使用。
    print("local_rank:", opt.local_rank)

    # 开启CuDNN的benchmark模式，可加速训练。
    cudnn.benchmark = True

    # 如果设置了opt.deterministic，则进行确定性训练设置。
    if opt.deterministic:
        cudnn.benchmark = False  # 关闭benchmark模式
        cudnn.deterministic = True  # 开启CuDNN的确定性模式
        torch.manual_seed(opt.local_rank)  # 设置PyTorch的随机种子为本地rank
        torch.set_printoptions(precision=10)  # 设置打印精度为10位小数

    # 设置当前GPU设备。
    torch.cuda.set_device(opt.local_rank)

    # 初始化分布式训练环境，使用NCCL作为后端。
    ...
    # 使用预定义的ConfigRandLA配置。
    rndla_cfg = ConfigRandLA

    # 创建SwinDePose模型，指定一些参数。
    model = SwinDePose(
    ...
    # 将模型中的Batch Normalization层转换为Sync Batch Normalization，用于分布式训练。
    ...
    # 如果在训练模型。
        ...
        if start_epoch == opt.n_total_epoch:
            _ = trainer.eval_epoch(val_loader, epoch=1)


if __name__ == "__main__":
    opt.world_size = opt.gpus * opt.nodes
    all_classes = opt.linemod_cls
    train()
