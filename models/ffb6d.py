...
class FFB6D(nn.Module):
    ...
    def forward(
        ...
        rgb_emb = self.cnn_pre_stages(inputs['rgb'])  # stride = 2, [bs, c, 240, 320]   inputs['rgb']:(1,3,480,640) rgb_emb:(1,64,120,160)
        # rndla pre
        xyz, p_emb = self._break_up_pc(inputs['cld_rgb_nrm'])   # inputs['cld_rgb_nrm']: (1,9,12800)  xyz:(1,12800,3)  p_emb:(1,6,12800)
        p_emb = inputs['cld_rgb_nrm']   #  p_emb:(1,9,12800)
        p_emb = self.rndla_pre_stages(p_emb)   # p_emb:(1,8,19200)
        p_emb = p_emb.unsqueeze(dim=3)  # Batch*channel*npoints*1   p_emb:(1,8,19200,1)

        # ###################### encoding stages #############################
        ds_emb = []
        for i_ds in range(4):
            # encode rgb downsampled feature
            rgb_emb0 = self.cnn_ds_stages[i_ds](rgb_emb)   # rgb_emb0:(1,64,120,160)
            ...
            f_sampled_i = self.random_sample(f_encoder_i, inputs['cld_sub_idx%d' % i_ds])   #(1,64,3200,1)
            p_emb0 = f_sampled_i #p_emb0:(1,64,3200,1)
            if i_ds == 0:
                ds_emb.append(f_encoder_i)

            # fuse point feauture to rgb feature
            p2r_emb = self.ds_fuse_p2r_pre_layers[i_ds](p_emb0)   #(1,64,3200,1)
            p2r_emb = self.nearest_interpolation(
                p2r_emb, inputs['p2r_ds_nei_idx%d' % i_ds]
            )   #(1,64,19200,1)
            p2r_emb = p2r_emb.view(bs, -1, hr, wr)    #(1,64,120,160)
            ...
            ).view(bs, c, -1, 1)   #(1,64,3200,1)
            r2p_emb = self.ds_fuse_r2p_pre_layers[i_ds](r2p_emb)   #(1,64,3200,1)
